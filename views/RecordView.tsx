      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
            ...
            if (err.message && err.message.includes("database")) {
               setError("Study generated but failed to save to database. Please check your connection.");
            } else {
    ...
    <div className="flex flex-col items-center justify-center h-full px-6 py-8 pb-24 space-y-6 relative">
      ...
      <div className="w-full max-w-md bg-gray-800 rounded-2xl p-6 border border-gray-700 shadow-xl relative z-0">
        ...
        <div className="flex justify-center">
          {!isRecording && !isProcessing ? (
            <button onClick={startRecording} className="group relative flex h-20 w-20 items-center justify-center rounded-full bg-red-600 transition-all hover:bg-red-700 focus:outline-none focus:ring-4 focus:ring-red-500/50 active:scale-95 shadow-lg">
               <div className="h-8 w-8 rounded-full bg-white shadow-sm"></div>
            </button>
          ) : isRecording ? (
            <button onClick={stopRecording} className="group relative flex h-20 w-20 items-center justify-center rounded-full bg-gray-700 border-2 border-red-500 transition-all hover:bg-gray-600 focus:outline-none active:scale-95 shadow-lg">
      ...
      {!isRecording && !isProcessing && (
          <div className="w-full max-w-md grid grid-cols-2 gap-3">
              <button onClick={() => fileInputRef.current?.click()} className="flex flex-col items-center justify-center space-y-1 p-3 rounded-xl bg-gray-800 border border-gray-700 hover:bg-gray-700 transition-colors text-gray-300 text-xs font-medium">
              ...
              <button onClick={() => initiateImageCapture('STUDY')} className="flex flex-col items-center justify-center space-y-1 p-3 rounded-xl bg-gray-800 border border-gray-700 hover:bg-gray-700 transition-colors text-gray-300 text-xs font-medium">
                 <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                 <span>Scan Notes</span>
              </button>
              <button onClick={() => setShowTranscriptModal(true)} className="flex flex-col items-center justify-center space-y-1 p-3 rounded-xl bg-gray-800 border border-gray-700 hover:bg-gray-700 transition-colors text-gray-300 text-xs font-medium">
                 <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                 <span>Paste Text</span>
              </button>
              <button onClick={() => initiateImageCapture('BULLETIN')} className="flex flex-col items-center justify-center space-y-1 p-3 rounded-xl bg-gray-800 border border-gray-700 hover:bg-gray-700 transition-colors text-gray-300 text-xs font-medium">
